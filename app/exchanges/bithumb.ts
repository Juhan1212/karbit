...
export class BithumbAdapter extends ExchangeAdapter {
  static readonly BASE_URL = "https://api.bithumb.com";
  ...
  async getBalance(): Promise<BalanceResult> {
    ...
    try {
      const res = await axios.get(`${BithumbAdapter.BASE_URL}/v1/accounts`, {
        headers,
      });
      // KRW 잔액 반환 (없으면 0)
  ...
  async placeOrder(params: OrderRequest): Promise<string> {
    try {
      ...
      const response = await axios.post(
        `${BithumbAdapter.BASE_URL}/v1/orders`,
        orderData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async getOrder(orderId: string, symbol?: string): Promise<OrderResult> {
    try {
      ...
      const response = await axios.get(
        `${BithumbAdapter.BASE_URL}/v1/order?${encoded}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
  ...
  async getTicker(symbol: string): Promise<TickerResult> {
    try {
      const response = await axios.get(
        `${BithumbAdapter.BASE_URL}/v1/ticker?markets=KRW-${symbol}`
      );
